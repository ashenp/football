...
abstract class Request {
	...
	public static function factory($config = null) {
		 ...
		 if($adapterRequest != '') {
		 	$class = 'Request_'.ucfirst($adapterRequest['controller']);
		 	return  new $class($adapterRequest);
		 }
		 //写到日志里
		 
	 	echo '当前域名信息 : '.$_SERVER['HTTP_HOST'].' : 当前域名不允许接入本系统.';
	 	exit;
	}
	
	//通用操作(一般是不带前缀的操作方法)
	abstract function publicService();
	
	//action 操作服务(带action前缀的操作)
	abstract function actionService();
	
	//system 系统操作服务(系统整体需要进行的操作)
	abstract function systemService();
	...
	public function __construct($config) {
		$this->parse();
		$this->_controller = ucfirst($config['controller']).'_'.$this->_controller;
	...
	private function interceptor() {
	    ...
	    if($adapterRequest['controller'] == 'api') {
	        ...
	        if($data != '') {
                ...
                $encrypt = new Encrypt($key, ENCRYPT_IV);
