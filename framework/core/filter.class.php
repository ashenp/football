...
abstract class Filter
{
    ...
    public static  function telephone($str){  
        if(preg_match("/^\d{3}|\d{4}|\d{5}-\d{7}|\d{8}$/", $str)) return true;  
        else return false;  
    }  
  
    // 匹配手机格式  
    public static  function mobile($str){  
        if(preg_match("/^1[0-9]{10}$/", $str)) return true;  
        else return false;  
    }  
    ...
    public static  function username($str){  
        if(preg_match("/^[a-z|A-Z]\w{3,15}$/", $str)) return true;  
        else return false;  
    }  
	...
	public static function password($str) {
		if(preg_match("/^(([a-z|A-Z|0-9])|(\.)|(\_)){6,16}$/", $str)) return true;  
        else return false;  
	}
	
	// 验证邮箱
	public static function zip($str) {
		if(preg_match("/^[0-9]{6}$/", $str)) return true;  
        else return false;  
	}
	
	// 验证区号 (验证固话部分)
	public static function area($str) {
		if(preg_match("/^[0-9]{3,6}$/", $str)) return true;  
        else return false;  
	}
	
	// 验证电话 (验证固话部分)
	public static function phone($str) {
		if(preg_match("/^[0-9]{7,8}$/", $str)) return true;  
        else return false;  
	}
	
	// 验证码 格式验证
	public static function code($str) {
		if(preg_match("/^[0-9|A-Z|a-z]{6}$/", $str)) return true;  
        else return false;  
	}
	...
	public static function price($str) {
		if(preg_match("/^(\d+)(\.?)(\d{0,2})$/", $str)) return true;  
        else return false;  
	}
}
